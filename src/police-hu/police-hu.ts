...
export const POLICE_HU_INFO_BASE_URL = 'http://www.police.hu/hu/hirek-es-informaciok/hatarinfo';
...
export const CROSSING_INFO_QUERY_PARAMS: Readonly<{ [K in Country]: string }> = Object.freeze({
...
export function extractLocationNames(crossingText: string): [string, string] {
  ...
  // in some cases the '–' character is used, so that must be changed to the standard '-' char.
  // an example for this is 'Bácsszentgyörgy–Raština '
  const crossingTextRemoveTrailingWhitespaces = crossingText.replace('–', '-').trim();
  let text;
  ...
  if (crossingParts.length > EXPECTED_NUMBER_OF_NAME_SEGMENTS) {
    // there a crossings with weird name, such as'Hegyeshalom-Nickelsdorf közút (1-es főút)'
